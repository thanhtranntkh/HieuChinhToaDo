...
st.set_page_config(layout="wide", page_title="Hiệu Chỉnh & Sắp Xếp Tọa Độ VN-2000")
...
st.title("🌐 Công Cụ Hiệu Chỉnh & Khép Kín Mốc Tọa Độ VN-2000")
st.markdown("Phát hiện đứt đoạn hình học, cho phép chọn mốc kết nối, sắp xếp lại thứ tự tuyến và lưu lại file Excel.")
...
def analyze_data(df, kinh_tuyen_truc):
    ...
    for i in range(1, len(df)):
        ...
        # Ngưỡng phát hiện đứt đoạn: khoảng cách giữa 2 mốc liên tiếp > 200m
        if dist > 200:
            df.at[i, 'Cảnh báo'] += f"🚨 Đứt đoạn ({dist}m)! "
            broken_segments.append({
                'from_stt': i, # STT trước (tính từ 1)
                'to_stt': i + 1,
                'distance': dist
            })
...
with st.sidebar:
    ...
    noi_dau_cuoi = st.checkbox("🔗 Tự động nối điểm đầu và điểm cuối", value=False)
    ...
    if 'X' not in raw_df.columns or 'Y' not in raw_df.columns:
        ...
        with col_data:
            if broken_segments:
                st.markdown(f"""
                <div class="alert-box">
                    <strong>🚨 PHÁT HIỆN {len(broken_segments)} ĐOẠN ĐỨT ĐOẠN HÌNH HỌC!</strong><br>
                    Các mốc đang bị nhảy cóc thứ tự. Hãy sử dụng công cụ bên dưới để kết nối lại hoặc sắp xếp lại thứ tự mốc trong file.
                </div>
                """, unsafe_allow_html=True)
            else:
                st.markdown("""
                <div class="success-box">
                    <strong>✅ Tuyến liền mạch!</strong> Không phát hiện đứt đoạn khoảng cách lớn.
                </div>
                """, unsafe_allow_html=True)
        ...
        with col_map:
            st.subheader("🗺️ Bản đồ tương tác & Trực quan hóa đoạn nối")
            st.caption("💡 Đường màu vàng: Tuyến mặc định theo STT Excel | Đường màu Cyan: Đoạn nối bổ sung/hiệu chỉnh.")
            ...
            if lat_lons:
                ...
                draw = plugins.Draw(export=False, draw_options={'polyline': False, 'polygon': True, 'rectangle': True, 'circle': False, 'marker': False, 'circlemarker': False}, edit_options={'edit': False})
                draw.add_to(m)
                ...
                # Vẽ các đường nối tùy chỉnh (Màu Cyan đậm)
                for pair in st.session_state['custom_connections']:
                    ...
                    if not row_1.empty and not row_2.empty:
                        try:
                            ...
                            folium.PolyLine([[lat1, lon1], [lat2, lon2]], color="cyan", weight=6, tooltip=f"Nối hiệu chỉnh: STT {stt_1} ↔ STT {stt_2}").add_to(m)
                        except:
                            pass
        ...
        with col_data:
            ...
            # --- CÔNG CỤ KẾT NỐI MỐC & SẮP XẾP LẠI THỨ TỰ ---
            with st.expander("🔗 Kết nối Mốc đứt đoạn (VD: 948 và 639)", expanded=True):
                st.caption("Chọn 2 mốc bất kỳ để hệ thống vẽ đường nối chuyên biệt hoặc đưa mốc đó lên liền kề nhau trong file.")
                all_stts = list(df_analyzed['STT'])
                
                pt_a = st.selectbox("Mốc bắt đầu (A):", options=[None] + all_stts, format_func=lambda x: f"STT {x}" if x is not None else "-- Chọn mốc A --", key="pt_a")
                pt_b = st.selectbox("Mốc kết thúc (B):", options=[None] + all_stts, format_func=lambda x: f"STT {x}" if x is not None else "-- Chọn mốc B --", key="pt_b")
                
                col_b1, col_b2 = st.columns(2)
                with col_b1:
                    if st.button("➕ Thêm đường nối vẽ"):
                        if pt_a and pt_b and (pt_a, pt_b) not in st.session_state['custom_connections']:
                            ...
                            st.rerun()
                with col_b2:
                    if st.button("🔄 Sắp xếp lại file"):
                        if pt_a and pt_b:
                            # Đưa dòng chứa pt_b nằm ngay sau dòng chứa pt_a trong dataframe thực tế
                            ...
                            if 0 <= idx_a < len(current_raw) and 0 <= idx_b < len(current_raw):
                                ...
                                # Chèn sau idx_a
                                insert_pos = idx_a + 1 if idx_a < idx_b else idx_a
                                part1 = current_raw.iloc[:insert_pos]
                                part2 = current_raw.iloc[insert_pos:]
                                st.session_state['df_current'] = pd.concat([part1, row_b, part2]).reset_index(drop=True)
                                st.success(f"Đã dời mốc STT {pt_b} lên liền sau mốc STT {pt_a} thành công!")
                                st.rerun()
                
                if st.button("🗑️ Xóa hết các đường nối đã vẽ"):
                    st.session_state['custom_connections'] = []
                    st.rerun()
            ...
            # XUẤT FILE SAU KHI HIỆU CHỈNH VÀ LƯU LẠI
            st.markdown("---")
            output = io.BytesIO()
